 ...
 * It is read from the QA_VIEWER_PASSWORD env var (which must be set via .env.branch only,
...
async function createAdminDirect() {
  ...
  const qaEmail = (process.env.QA_VIEWER_EMAIL || 'qa-admin@bait.invalid').toLowerCase().trim();
  ...
  if (!qaPassword || qaPassword.length < 14 || qaPassword.length > 128) {
    console.error('❌ QA_VIEWER_PASSWORD must be 14-128 chars. Set in .env.branch only.');
    process.exit(1);
  }
  ...
  if (!u.hash_valid || u.role !== 'VIEWER' || !u.active) {
