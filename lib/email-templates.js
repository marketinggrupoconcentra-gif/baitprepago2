...
function renderReportEmail(data, period) {
  const html = layout(`
    ...
    <table style="width: 100%; border-collapse: collapse; margin-top: 24px;">
      ...
      <tr>
        <td style="padding: 12px; border-bottom: 1px solid #eee;"><strong>Sin atribución</strong></td>
        <td style="padding: 12px; border-bottom: 1px solid #eee; text-align: right;">${data.unassignedLeads}</td>
      </tr>
    </table>
    <div style="text-align: center;">
      <a href="${process.env.APP_BASE_URL || process.env.PUBLIC_URL || 'https://baitprepago.com'}/admin/" class="btn" style="color: #ffffff;">Ver detalles en el panel</a>
    </div>
  `);
...
Sin atribución: ${data.unassignedLeads}
  `.trim();
...
function renderPasswordChangedEmail(dateString) {
  const html = layout(`
    ...
    <div style="text-align: center; margin-top: 24px;">
      <a href="${process.env.APP_BASE_URL || process.env.PUBLIC_URL || 'https://baitprepago.com'}/admin/" class="btn" style="color: #ffffff;">Iniciar sesión</a>
    </div>
