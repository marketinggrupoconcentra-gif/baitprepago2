 ...
 */
export async function createCaptchaChallenge(sql, env = process.env, clientHash = null) {
  ...
  await sql`
    INSERT INTO captcha_challenges (id, answer_hash, expires_at, client_hash)
    VALUES (${challengeId}, ${answerHash}, ${expiresAt.toISOString()}, ${clientHash})
  `;
...
// Challenge-creation abuse control. Deliberately holgado for a human
// refreshing the image several times, tight enough to block scripted bursts.
// Window/limit are exported so tests can assert on the same constants.
export const CAPTCHA_CHALLENGE_RATE_LIMIT_WINDOW_MINUTES = 10;
export const CAPTCHA_CHALLENGE_RATE_LIMIT_MAX = 20;

/**
 * Identifies a client for rate-limiting purposes without storing the raw IP
 * anywhere: only an HMAC-SHA256(CAPTCHA_PEPPER, ip) is ever persisted.
 */
export function hashClientIdentifier(pepper, ip) {
  return crypto.createHmac('sha256', pepper).update(`captcha-rl:${ip || 'unknown'}`).digest('hex');
}

/**
 * Returns { allowed: true } or { allowed: false } based on how many
 * challenges this client_hash has created in the trailing rate-limit
 * window. Fails closed (not allowed) if CAPTCHA_PEPPER is missing, mirroring
 * every other CAPTCHA entry point.
 */
export async function checkCaptchaChallengeRateLimit(sql, ip, env = process.env) {
  let pepper;
  try {
    pepper = getCaptchaPepper(env);
  } catch {
    return { allowed: false, clientHash: null };
  }

  const clientHash = hashClientIdentifier(pepper, ip);

  const rows = await sql`
    SELECT count(*)::int AS count
    FROM captcha_challenges
    WHERE client_hash = ${clientHash}
      AND created_at > NOW() - (${CAPTCHA_CHALLENGE_RATE_LIMIT_WINDOW_MINUTES} * INTERVAL '1 minute')
  `;

  const count = rows?.[0]?.count ?? 0;
  return { allowed: count < CAPTCHA_CHALLENGE_RATE_LIMIT_MAX, clientHash };
}
